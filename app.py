...
# Ruta al volumen persistente de Railway
INSTANCE_PATH = '/data'
DATABASE_NAME = 'optic_clarity.db'
app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{os.path.join(INSTANCE_PATH, DATABASE_NAME)}'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Crear carpeta del volumen si no existe (Railway ya la crea, pero por seguridad)
...
with app.app_context():
    ...
    # Insertar datos de ejemplo solo si la tabla de productos está vacía
    if Product.query.count() == 0:
        ...
        lenses = [
            Product(type='lens', name='Digital HD Clear', brand='OpticClarity Lab', price=180.00,
                    description='Lentes con filtro de luz azul y anti-reflejo.',
            ...
            Product(type='lens', name='Progressive Elite', brand='OpticClarity Lab', price=320.00,
                    description='Lentes progresivos con transición suave.',
                    image_url='https://lh3.googleusercontent.com/aida-public/AB6AXuA_3T-M9mbyXkkRkLVQg3yf1sDPZu16pWkpO9s6nNXf7nXY3t0WUi821QcdxNsOuv2nZbXp1UcxVR9EwJ4_LuqQLpO1waiqOaN8UGT3wPfB-XHq3XdAdnPinae8CR_ZPulT5hAExtt1A7WibN4rsLj3YuEbdTxFzjnobVcvXO5rdVgFf-S3sy0Z2f8t6ZmgGkflE9liiLutle_WTo3U5t7wTWee32VnZ7ZFytfEAoZwq_FghLdONNjBlmLqj0QNWkyYBY4_tvOt4xM',
        ...
        db.session.commit()
        print("✅ Base de datos inicializada automáticamente con datos de ejemplo.")
    else:
        print("ℹ️ La base de datos ya contiene datos. No se insertaron duplicados.")
...
# ---------- Comando CLI para inicializar datos (opcional) ----------
@app.cli.command('init-db')
def init_db_command():
    """Crea las tablas y carga datos si la base de datos está vacía."""
    # db.drop_all() # Descomenta solo si quieres borrar todo en cada reinicio
    db.create_all()
    
    # Solo insertar si no hay productos (evita duplicados al reiniciar)
    if Product.query.count() == 0:
        frames = [
            Product(type='frame', name='Aero Classic', brand='VOGUE OPTICS', price=285.00,
                    image_url='https://lh3.googleusercontent.com/aida-public/AB6AXuCOo5oxuaFSeJh695485oN5jMcHYrw5Q9Yu8nEMwqp9APlfcInxcQ-DLbVgr1ZGTW_cSrunhlvOY73nIz2rIoUFIl92olJKqNN00noU6OC7Mj1fK8FQGnHCrdq_iG-k3ZHtpztk_oY3bXIRx_b4pABNCa_9XLLd5YW9lKlJqxOTXk-A6ToH1FBFxtle6EZnXcu37kog8RbzQV6kTAiaEAGhwLMHXtkqNZ4UpRdiwauXI9x_hQtxyRVKY8qAsx7LJgWc3ey9hPj_30c',
        ...
        db.session.add_all(announcements)
        db.session.commit()
        print('Tablas creadas y datos de ejemplo cargados con éxito.')
    else:
        print('La base de datos ya contiene datos. No se realizaron cambios.')

if __name__ == '__main__':
    # Nota: En Railway esto solo se usa si no defines un Start Command con Gunicorn
